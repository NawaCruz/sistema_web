...
@section('content')
    ...
    <div class="d-flex justify-content-end mb-3">
        <button class="btn btn-sm btn-primary shadow-sm" data-toggle="modal" data-target="#modalCrearProducto">
                            ...
                            @foreach ($productos as $producto)
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <button class="btn btn-sm btn-warning" data-toggle="modal" data-target="#modalEditarProducto{{ $producto->id }}">
                                            <i class="fas fa-edit"></i>
                                        </button>
                                        <button class="btn btn-sm btn-danger" data-toggle="modal" data-target="#modalEliminarProducto{{ $producto->id }}">
    ...
    @include('empleado.productos.modales.crear')
    @include('empleado.productos.modales.editar')
    @include('empleado.productos.modales.eliminar')
@endsection
...
@push('scripts')
    <script>
        $(document).ready(function () {
            ...
            // CANCELAR CREACIÓN
            $(document).on('click', '.btn-cancelar-crear-producto', function () {
                let modal = $('#modalCrearProducto');
        ...
        $(document).ready(function () {
            ...
            $(document).on('submit', '.form-editar-producto', function (e) {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        // Cerrar el modal
                        $('#modalEditarProducto' + id).modal('hide');
        ...
        $(document).ready(function () {
            ...
            $(document).on('submit', '.form-eliminar-producto', function (e) {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        // Cerrar el modal
                        $('#modalEliminarProducto' + id).modal('hide');
            ...
            $(document).on('click', '.btn-cancelar-eliminar-producto-empleado', function () {
                let modal = $(this).closest('.modal');
                modal.find('form')[0].reset(); // Limpia por si acaso
            });
        });

        $(document).ready(function () {
            // ELIMINAR PRODUCTO (Empleado)
            $(document).on('submit', '.form-eliminar-producto-empleado', function (e) {
                e.preventDefault();
                ...
                $.ajax({
                    ...
                    success: function () {
                        ...
                        // Recargar el contenido principal sin refrescar toda la página
                        $.get(window.location.href, function (data) {
                            $('#main-content').html($(data).find('#main-content').html());
                        });
                    },

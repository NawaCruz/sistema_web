...
@section('content')
    ...
    <div class="d-flex justify-content-end mb-3">
        <button class="btn btn-sm btn-primary shadow-sm" data-toggle="modal" data-target="#modalCrearCompraEmpleado">
                                ...
                                @foreach ($compras as $compra)
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <!-- Editar -->
                                            <button class="btn btn-sm btn-warning" data-toggle="modal" data-target="#modalEditarCompraEmpleado{{ $compra->id }}">
                                                <i class="fas fa-edit"></i>
                                            </button>
                                            <!-- Eliminar -->
                                            <button type="button" class="btn btn-sm btn-danger" data-toggle="modal" data-target="#confirmarEliminarCompraEmpleado{{ $compra->id }}">
    ...
    <div class="modal fade" id="modalCrearCompraEmpleado" tabindex="-1" role="dialog" aria-labelledby="modalCrearCompraLabel" aria-hidden="true">
        <div class="modal-dialog modal-md" role="document">
            <div class="modal-content">
                <form id="form-crear-compra-empleado" method="POST">
                    @csrf
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" name="total" class="form-control" step="0.1" min="0" required>
                            <span class="text-danger error-totalCompra"></span>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="submit" class="btn btn-primary">Guardar</button>
                        <button type="button" class="btn btn-secondary btn-cancelar-nueva-compra-empleado" data-dismiss="modal">Cancelar</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <!-- MODAL: Editar compra -->
    @foreach ($compras as $compra)
    <div class="modal fade" id="modalEditarCompraEmpleado{{ $compra->id }}" tabindex="-1" role="dialog" aria-labelledby="modalEditarCompraLabel{{ $compra->id }}" aria-hidden="true">
        <div class="modal-dialog modal-md" role="document">
            <div class="modal-content">
                <form class="form-editar-compra-empleado" method="POST" data-id="{{ $compra->id }}">
                    @csrf
                    @method('PUT')
                    <div class="modal-header">
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" class="btn btn-secondary btn-cancelar-edicion-compra-empleado" data-dismiss="modal">Cancelar</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <div class="modal fade" id="confirmarEliminarCompraEmpleado{{ $compra->id }}" tabindex="-1" role="dialog" aria-labelledby="modalLabelEliminar{{ $compra->id }}" aria-hidden="true">
        <div class="modal-dialog" role="document" style="margin-top: 80px; transform: translateX(80px);">
            <div class="modal-content">
            <form class="form-eliminar-compra-empleado" data-id="{{ $compra->id }}">
                @csrf
                @method('DELETE')
                ...
                <button type="button" class="btn btn-secondary btn-cancelar-eliminar-compra-empleado" data-dismiss="modal">Cancelar</button>
                </div>
            </form>
            </div>
        </div>
...
@push('scripts')
    <script>
        $(document).on('submit', '#form-crear-compra-empleado', function (e) {
            e.preventDefault();
            ...
            $.ajax({
                ...
                success: function (response) {
                    // Cerrar el modal
                    $('#modalCrearCompraEmpleado').modal('hide');
                    // Limpiar formulario
                    form[0].reset();
        ...
        // EDITAR compra
        $(document).on('submit', '.form-editar-compra-empleado', function (e) {
            e.preventDefault();
            ...
            $.ajax({
                ...
                success: function (response) {
                    // Cerrar modal
                    $('#modalEditarCompraEmpleado' + id).modal('hide');
                    // Limpiar formulario
                    form[0].reset();
        ...
        $(document).on('click', '.btn-cancelar-edicion-compra-empleado', function () {
        ...
        $(document).on('click', '.btn-cancelar-nueva-compra-empleado', function () {
            let modal = $('#modalCrearCompraEmpleado');
            modal.find('form')[0].reset(); // Limpia campos
            modal.find('.is-invalid').removeClass('is-invalid'); // Limpia errores
            modal.find('.text-danger').text(''); // Limpia mensajes de error
        });
        ...
        $(document).ready(function () {
            $(document).on('submit', '.form-eliminar-compra-empleado', function (e) {
                e.preventDefault();
                ...
                $.ajax({
                    ...
                    success: function () {
                        $('#confirmarEliminarCompraEmpleado' + id).modal('hide');

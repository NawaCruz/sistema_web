...
@section('content')
    ...
    <div class="d-flex justify-content-between align-items-center mb-3">
        <a href="{{ route(auth()->user()->role . '.compras.index') }}" class="btn btn-sm btn-primary shadow-sm">
            <i class="fas fa-arrow-left"></i> Volver
        </a>
        <button class="btn btn-sm btn-primary shadow-sm" data-toggle="modal" data-target="#modalCrearDetalleCompraAdmin">
                        ...
                        <tbody>
                            @foreach ($detalles as $detalle)
                                <tr>
                                    ...
                                    <td>
                                        <!-- Editar -->
                                        <button class="btn btn-sm btn-warning" data-toggle="modal" data-target="#modalEditarDetalleCompraAdmin{{ $detalle->id }}">
                                            <i class="fas fa-edit"></i>
                                        </button>
                                        <!-- Eliminar -->
                                        <button type="button" class="btn btn-sm btn-danger" data-toggle="modal" data-target="#modalEliminarDetalleCompraAdmin{{ $detalle->id }}">
    ...
    <div class="modal fade" id="modalCrearDetalleCompraAdmin" tabindex="-1" role="dialog" aria-labelledby="modalCrearProveedorLabel" aria-hidden="true">
        <div class="modal-dialog modal-md" role="document">
            <div class="modal-content">
                <form id="form-crear-detalle-compra-admin" method="POST">
                    @csrf
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            ...
                            <p class="form-control bg-light mb-2" id="crear-subtotal-mostrar-admin">S/ 0.00</p>
                            <input type="hidden" name="subtotal" id="crear-subtotal-enviar-admin">
                            <span class="text-danger error-subtotal"></span>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="submit" class="btn btn-primary">Guardar</button>
                        <button type="button" class="btn btn-secondary btn-cancelar-nuevo-detalle-compra-admin" data-dismiss="modal">Cancelar</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    @foreach ($detalles as $detalle)
    <div class="modal fade" id="modalEditarDetalleCompraAdmin{{ $detalle->id}}" tabindex="-1" role="dialog" aria-labelledby="modalEditarDetalleLabel{{ $detalle->id}}" aria-hidden="true">
        <div class="modal-dialog modal-md" role="document">
            <div class="modal-content">
                <form class="form-editar-detalle-compra-admin" method="POST" data-id="{{ $detalle->id }}">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            <label>Subtotal</label>
                            <p class="form-control bg-light mb-2" id="editar-subtotal-mostrar-admin-{{ $detalle->id }}">S/ {{ number_format($detalle->subtotal, 2) }}</p>
                            <input type="hidden" name="subtotal" id="editar-subtotal-enviar-admin-{{ $detalle->id }}" value="{{ $detalle->subtotal }}">
                            <span class="text-danger error-subtotal"></span>
                        </div>
                    </div>
                    <div class="modal-footer">
    ...
    <div class="modal fade" id="modalEliminarDetalleCompraAdmin{{ $detalle->id }}" tabindex="-1" role="dialog" aria-labelledby="modalLabelEliminarDetalle{{ $detalle->id }}" aria-hidden="true">
        <div class="modal-dialog" role="document" style="margin-top: 80px; transform: translateX(80px);">
            <div class="modal-content">
            <form class="form-eliminar-detalle-compra-admin" data-id="{{ $detalle->id }}">
                @csrf
                @method('DELETE')
...
@push('scripts')  
    <script>
        $(document).on('input', '#form-crear-detalle-compra-admin input[name="cantidad"], #form-crear-detalle-compra-admin input[name="precio_unitario"]', function () {
            let cantidad = parseFloat($('#form-crear-detalle-compra-admin input[name="cantidad"]').val()) || 0;
            let precio = parseFloat($('#form-crear-detalle-compra-admin input[name="precio_unitario"]').val()) || 0;
            let subtotal = (cantidad * precio).toFixed(2)

            $('#crear-subtotal-mostrar-admin').text('S/ ' + subtotal);
            $('#crear-subtotal-enviar-admin').val(subtotal);
        });

        $(document).on('input', '.form-editar-detalle-compra-admin input[name="cantidad"], .form-editar-detalle-compra-admin input[name="precio_unitario"]', function () {
            ...
            $('#editar-subtotal-mostrar-admin-' + id).text('S/ ' + subtotal);
            $('#editar-subtotal-enviar-admin-' + id).val(subtotal);
        });
        ...
        $(document).ready(function () {
            // Envío AJAX
            $(document).on('submit', '#form-crear-detalle-compra-admin', function (e) {
                e.preventDefault();
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        $('#modalCrearDetalleCompraAdmin').modal('hide');
        ...
        $(document).ready(function () {
            ...
            $(document).on('input', '.form-editar-detalle-compra-admin input[name="cantidad"], .form-editar-detalle-compra-admin input[name="precio_unitario"]', function () {
                ...
                let url = `/admin/detalleCompras/${id}`;
            ...
            $(document).on('submit', '.form-editar-detalle-compra-admin', function (e) {
                e.preventDefault();
        ...
        $(document).on('click', '.btn-cancelar-nuevo-detalle-compra-admin', function () {
            let form = $('#form-crear-detalle-compra-admin');
            form[0].reset();
            form.find('.is-invalid').removeClass('is-invalid');
            form.find('.text-danger').text('');
            $('#crear-subtotal-mostrar-admin').text('S/ 0.00');
            $('#crear-subtotal-enviar-admin').val('');
        });

        // Cancelar edición de detalle de compra
        $(document).on('click', '.modal[id^="modalEditarDetalleCompraAdmin"] .btn.btn-secondary[data-dismiss="modal"]', function () {
            const $modal = $(this).closest('.modal');
            const $form  = $modal.find('.form-editar-detalle-compra-admin');
            ...
            $('#editar-subtotal-mostrar-admin-' + id).text('S/ ' + subtotalDef);
            $('#editar-subtotal-enviar-admin-' + id).val(subtotalDef);
        });

        // Eliminar DETALLE de compra
        $(document).on('submit', '.form-eliminar-detalle-compra-admin', function (e) {
        e.preventDefault();
        ...
        const url   = `/${role}/detalleCompras/${id}`;
            ...
            $.ajax({
                ...
                success: function () {
                $(`#modalEliminarDetalleCompraAdmin${id}`).modal('hide');
